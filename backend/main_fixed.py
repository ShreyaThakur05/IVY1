...
async def clone_voice(
    ...
    try:
        print(f"Cloning voice '{voice_name}' for persona ID '{persona_id}'...")
        ...
        # Store voice using persona_id as key (CRITICAL FIX)
        voice_personas[persona_id] = {
            "voice_id": voice_id,
        ...
        print(f"SUCCESS: Stored voice '{voice_name}' under persona_id '{persona_id}': {voice_id}")
...
async def handle_chat(
    ...
    try:
        print(f"Chat request: {persona_name} (ID: {persona_id}) for {interview_topic}")
        ...
        try:
            ...
            if len(history) == 0:
                prompt = f"""You are {persona_name}, conducting a {interview_topic} interview.
...
Keep it under 35 words. DO NOT repeat previous questions."""
        ...
        # 5. Get voice ID using persona_id (CRITICAL FIX)
        voice_id = "21m00Tcm4TlvDq8ikWAM"  # Default ElevenLabs voice
        
        print(f"Looking for voice for persona_id: '{persona_id}'")
        print(f"Available voices: {list(voice_personas.keys())}")
        
        if persona_id in voice_personas:
            voice_id = voice_personas[persona_id]['voice_id']
            print(f"SUCCESS: Using custom voice for {persona_name}: {voice_id}")
        else:
            print(f"No custom voice for '{persona_id}', using default")
...
async def start_interview(
    ...
    try:
        ...
        print(f"Starting interview: {persona_name} (ID: {persona_id}) for {interview_topic}")
        ...
        try:
            model = genai.GenerativeModel('gemini-1.5-flash')
            prompt = f"""You are {persona_name}, starting a {interview_topic} interview. 
...
Give a warm, brief welcome (under 30 words) and ask the candidate to introduce themselves."""
            ...
            print(f"AI error: {e}")
            opening_message = f"Hello! I'm {persona_name}. Welcome to your {interview_topic} interview. Please introduce yourself and tell me about your background."
        
        # Get voice using persona_id (CRITICAL FIX)
        voice_id = "21m00Tcm4TlvDq8ikWAM"  # Default ElevenLabs voice
